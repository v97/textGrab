...
infoAboutRelevantItem = requests.get('https://api.nal.usda.gov/ndb/reports/?ndbno=' + str(mostRelevantItemId) + '&type=f&format=json&api_key=' + api_key).json()

#get rid of bullcrap
nutrients = {nutrient['name'] : str(nutrient['value']) + ' ' + str(nutrient['unit']) for nutrient in infoAboutRelevantItem['report']['food']['nutrients']}
